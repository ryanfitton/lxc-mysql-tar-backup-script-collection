...
##   2: MySQL Host Address. This is the internal container address.
##   3: MySQL Root User.
...
MYSQL_HOST="$2"             # MySQL Host Address. This is the internal container address.
MYSQL_USER="$3"             # MySQL Root User.
MYSQL_PASSWORD="$4"         # MySQL Root Password.

# Backup options
FTP_SAVEPATH="$GLOBAL_REMOTE_FTP_LOCATION/$NAME/sql"     # The path for where the .sql file should be uploaded on the FTP server.
...
then
    ...
    then
        ...
        then
            ...
            FILENAME="$NAME-$MYSQL_USER-all-databases-mysqldump-$HOSTNAME-$NOW.sql"     # Filename (name.mysql-backup.hostname.timestamp)
